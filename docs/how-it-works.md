...
- Pushing is publishing, once the repo is connected to Cloudflare: Workers watches it and rebuilds the live site on every push, in about a minute. That connection is a one-time setup step, and it fails silently if it was skipped — `docs/deploy-cloudflare.md` says how to check.
